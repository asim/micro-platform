...
func (h *Handler) GetEvent(ctx context.Context, req *pb.GetEventRequest, rsp *pb.GetEventResponse) error {
	...
	if len(req.Event.Id) == 0 {
		return errors.BadRequest("go.micro.platform", "missing event ID")
	}
	...
	if len(records) == 0 {
		return errors.NotFound("go.micro.platform", "event not found")
	}

	// Decode the event
	if err := json.Unmarshal(records[0].Value, &rsp.Event); err != nil {
		return errors.InternalServerError("go.micro.platform", "unable to decode record")
	}
